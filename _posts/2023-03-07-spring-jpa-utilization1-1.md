...
## JPA 내장 타입
> JPA에서 새로운 값 타입을 정의해서 사용할 수 있는데 이것을 내장 타입이라고 합니다. 사용 방법은 간단합니다. 내장 타입으로 사용할 클래스에 **@Embeddable** 어노테이션을 붙여주고 내장 타입을 사용하는 각 필드 위에는 **@Embedded** 어노테이션을 붙여주면 됩니다. 이때 내장 타입으로 사용할 클래스에 Setter를 절대 열어두면 안됩니다. 그러면 굉장히 나중에 유지보수하기 어려워지기 때문에 파라미터를 넘겨받는 생성자에 의한 값 초기화만 열어두도록 해야합니다. 또한 JPA 스펙상 엔티티나 임베디드 타입은 자바 기본 생성자를 public 혹은 protected로 설정해놔야 하기 때문에 protected로 기본 생성자도 함께 만들어줘야 합니다. JPA가 이런 생성자를 필요로 하는 이유는 JPA 구현 라이브러리가 객체를 생성할 때 리플랙션 같은 기술을 사용하기 때문입니다.
