...
## 컨트롤러, 서비스, 리포지토리 계층 구분 팁
> 계층을 컨트롤러, 서비스, 리포지토리, 도메인으로 많이 나눠서 개발을 해왔는데 이때 주의할 점이 있습니다. 다들 알다시피 서비스에서는 비즈니스 로직을 주로 다루고 리포지토리에서는 데이터베이스 접근을 다룹니다. 따라서 컨트롤러에서는 가급적 직접 데이터베이스를 조회하거나 그런 로직을 짜지 말고 단순히 PK값과 같은 식별자 만을 서비스에 넘겨주고, 식별자 값을 넘겨받은 서비스 계층에서는 리포지토리를 활용해 데이터베이스에 접근하는 것이 중요합니다. 이때 JPA를 통한 데이터 변경 모든 로직들은 트랜잭션 단위로 이루어지기 때문에 서비스 계층에는 @Transactional 어노테이션이 붙어있어야 합니다.
